...
import { Ref } from 'vue'
import { tokenComp, presetComp } from './localStorage'
...
async function searchIP(nextboot: Ref, log: Function) {
  ...
  ips.forEach((ip: string) => {
    ...
    httpRequests.push(
      axios.get(`${lip}/`, {headers: { Authorization: `Bearer ${tokenComp.value}` }})
      .then(res => {
        if (res.status === 200 && res.data === "powercontrol") {
          connected.value = lip
          // error.value = ""
          getnextboot(nextboot)
        }  
      })
    )
  })
...
function setnextboot(presetName: "windows" | "linux", nextboot: Ref) {
  const nextpreset = presetComp.value[presetName]
  axios.post(`${connected.value}/setnextboot`, nextpreset, 
             {headers: { Authorization: `Bearer ${tokenComp.value}` }})
             .then(res => {
...
function getnextboot(nextboot: Ref) {
  axios.get(`${connected.value}/getnextboot`,
            ...
            .then(res => {
              if (res.status === 200) {
                // error.value = ""
                if (JSON.stringify(res.data) == JSON.stringify(presetComp.value.linux)) {
                  nextboot.value = "linux"
                } else if (JSON.stringify(res.data) == JSON.stringify(presetComp.value.windows)) {
